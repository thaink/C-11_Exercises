...
namespace ktstd2 {
    ...
    class list_map : private std::map<Key,list_map_base<std::pair<const Key,T>>, Compare, Allocator> {
    ...
    public:
        ...
        using base_type                 = std::map<Key,node_type, Compare, Allocator>;
        using size_type                 = typename base_type::size_type;

        using reference                 = value_type&;
        using const_reference           = const value_type&;
        //iterator types
        using iterator                  = list_map_iterator<value_type,value_type*,value_type&>;
        using const_iterator            = list_map_iterator<value_type, const value_type*, const value_type&>;
    ...
    //list_map::list_map(const Compare& compare, const Allocator& allocator)
        :mNode(),base_type(compare, allocator)
    {
        mNode.Next = &mNode;
        mNode.Prev = &mNode;
    }
    ...
    bool list_map<Key, T, Compare, Allocator>::push_front(const value_type& value)
    {
        ...
        auto res = base_type::emplace(value.first, temp);
        if (res.second) {
    ...
    void list_map<Key, T, Compare, Allocator>::pop_front()
    {
        ...
        auto res = base_type::erase(pnode->value.first);
    ...
    {
        auto fval = base_type::find(key);
        if(fval != base_type::end())//remember mMap.end() != end()
        {
    ...
    {
        auto fval = base_type::find((*position).first);
        if(fval != base_type::end())
        {
            auto idx = &(*fval).second;
            auto inext = idx->Next;
    ...
    bool list_map<Key, T, Compare, Allocator>::validate() const
    {
        ...
        for (auto idx=begin(); idx != end(); ++idx)
            ++count;
        if(base_type::size()!=count) return false;

        auto ibegin = (*base_type::begin()).second.Prev;
        auto iend   = (*base_type::rbegin()).second.Next;
